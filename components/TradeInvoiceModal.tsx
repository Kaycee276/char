...
export function TradeInvoiceModal({ isOpen, onClose, onTradeCreated }: TradeInvoiceModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 backdrop-blur-sm p-4 overflow-y-auto">
      <div className="w-full max-w-2xl neu-card rounded-3xl p-6 sm:p-8 space-y-6 relative border border-white/80 dark:border-white/10 my-8 animate-in fade-in zoom-in-95 duration-200">
        {/* Header */}
        <div className="flex items-center justify-between border-b border-slate-300/40 dark:border-slate-700/60 pb-4">
          <div>
            <h3 className="text-lg font-bold text-slate-900 dark:text-slate-100">New Cross-Continental Trade Contract</h3>
            <p className="text-xs text-slate-600 dark:text-slate-400 font-medium">Initiate settlement invoice with auto-yield escrow</p>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <select
                ...
                className="w-full px-3 py-2.5 rounded-2xl neu-input text-slate-900 dark:text-slate-100 text-xs font-bold"
              >
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div>
                ...
                <div className="relative w-48">
                  <span className="absolute left-3 top-2.5 text-xs text-slate-500 dark:text-slate-400 font-bold">$</span>
                  <input
                    type="number"
                    min={10}
                    step={10}
                    ...
                    className="w-full pl-7 pr-3 py-2 rounded-xl neu-card text-slate-950 dark:text-slate-100 text-sm font-black font-mono border border-white/80 dark:border-white/10"
                  />
                </div>
              </div>

              <div className="flex flex-wrap gap-4 text-xs">
                <div className="neu-card-sm px-3 py-2 rounded-xl border border-white/70 dark:border-white/10">
                  <span className="text-[10px] text-slate-600 dark:text-slate-400 block font-bold">Bolivia (BOB):</span>
                  <span className="font-black text-orange-700 dark:text-orange-400 font-mono">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {/* Exporter (Bolivia) */}
            <div className="neu-card-sm p-4 rounded-2xl border border-white/80 dark:border-white/10 space-y-2">
              <span className="text-[10px] font-extrabold uppercase tracking-wider text-orange-800 dark:text-orange-300 bg-orange-100 dark:bg-orange-950/60 px-2 py-0.5 rounded-full">
                Bolivian Seller / Exporter
              </span>
              <div>
                <label className="text-[11px] font-bold text-slate-700 dark:text-slate-300 block">Company / Name</label>
                <input
                  type="text"
                  value={sellerName}
                  onChange={(e) => setSellerName(e.target.value)}
                  className="w-full px-3 py-1.5 rounded-xl neu-input text-xs font-bold text-slate-900 dark:text-slate-100"
                />
              </div>
              <div>
                <label className="text-[11px] font-bold text-slate-700 dark:text-slate-300 block">City</label>
                <input
                  type="text"
                  value={sellerCity}
            ...
            {/* Importer (Africa) */}
            <div className="neu-card-sm p-4 rounded-2xl border border-white/80 dark:border-white/10 space-y-2">
              <span className="text-[10px] font-extrabold uppercase tracking-wider text-emerald-800 dark:text-emerald-300 bg-emerald-100 dark:bg-emerald-950/60 px-2 py-0.5 rounded-full">
                ...
                {(['Kenya', 'Nigeria', 'Ghana'] as const).map((c) => (
                  <button
                    ...
                    className={`py-1 rounded-xl text-[10px] font-bold transition-all ${
                      buyerCountry === c
                        ? 'neu-inset text-emerald-800 dark:text-emerald-300 font-black'
                        : 'neu-btn text-slate-700 dark:text-slate-300 font-semibold'
                    }`}
                  >
                    {c}
                  </button>
          ...
          <div className="neu-card-sm p-3.5 rounded-2xl flex items-center justify-between gap-3 text-xs bg-emerald-50/70 dark:bg-emerald-950/40 border border-emerald-300 dark:border-emerald-700/60">
            <span className="text-emerald-900 dark:text-emerald-300 font-black">
              Blend Yield Strategy:
            </span>
            <span className="text-emerald-800 dark:text-emerald-300 font-semibold">
              Funds earn <strong className="font-black text-emerald-950 dark:text-emerald-200">+7.8% APY</strong> in Blend USDC lending pool during transit.
            </span>
          </div>
          ...
          <div className="flex gap-3 pt-2">
            <button
              type="button"
              onClick={onClose}
              className="flex-1 py-3 px-4 rounded-2xl neu-btn text-slate-700 dark:text-slate-300 hover:text-slate-950 dark:hover:text-white text-xs font-bold"
            >
              Cancel
            </button>
            <button
              type="submit"
              className="flex-1 py-3 px-4 rounded-2xl neu-btn-primary text-white text-xs font-bold flex items-center justify-center shadow-neu-glow"
            >
              Issue Trade Contract
            </button>
          </div>
        </form>
      </div>
