...
export function TradeList() {
  ...
  const getStatusBadge = (status: TradeStatus) => {
    switch (status) {
      case 'CREATED':
        return (
          <span className="px-3 py-1 rounded-full text-xs font-extrabold neu-inset text-amber-700 dark:text-amber-400 flex items-center gap-2">
            <span className="w-2 h-2 rounded-full bg-amber-500 animate-pulse" /> Awaiting Funding
          </span>
        );
      case 'ESCROW_LOCKED':
        return (
          <span className="px-3 py-1 rounded-full text-xs font-extrabold neu-card-sm text-emerald-800 dark:text-emerald-300 bg-emerald-100/70 dark:bg-emerald-950/60 border border-emerald-300 dark:border-emerald-700/60 flex items-center gap-2">
            <span className="w-2 h-2 rounded-full bg-emerald-500 animate-ping" /> Escrow Locked &amp; Earning
          </span>
        );
      case 'RELEASED':
        return (
          <span className="px-3 py-1 rounded-full text-xs font-extrabold neu-card-sm text-indigo-800 dark:text-indigo-300 bg-indigo-100/70 dark:bg-indigo-950/60 border border-indigo-300 dark:border-indigo-700/60 flex items-center gap-2">
            <span className="w-2 h-2 rounded-full bg-indigo-500" /> Delivered (Ready for BOB)
          </span>
        );
      case 'OFFRAMPED_BOB':
        return (
          <span className="px-3 py-1 rounded-full text-xs font-black neu-inset text-orange-800 dark:text-orange-300 flex items-center gap-2">
    ...
    <section className="w-full max-w-7xl mx-auto px-4 sm:px-8 py-6 space-y-6">
      ...
      <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4">
        ...
        <div className="relative flex-1 max-w-md">
          <Search className="w-4 h-4 text-slate-500 dark:text-slate-400 absolute left-4 top-3.5" />
          <input
            type="text"
            placeholder="Search commodities, companies, or cities..."
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            className="w-full pl-11 pr-4 py-2.5 rounded-2xl neu-input text-xs font-semibold text-slate-900 dark:text-slate-100 placeholder:text-slate-500 dark:placeholder:text-slate-400"
          />
        </div>
        ...
        <button
          ...
          className="px-5 py-2.5 rounded-2xl neu-btn-primary text-white text-xs font-bold flex items-center justify-center gap-1.5 shadow-neu-glow hover:opacity-95"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {filteredTrades.map((trade) => {
          ...
          return (
            <div
              key={trade.id}
              className={`neu-card rounded-3xl p-6 sm:p-7 border border-white/80 dark:border-white/10 space-y-5 flex flex-col justify-between transition-all ${
            ...
            >
              <div>
                ...
                <div className="flex items-start justify-between gap-3 mb-2">
                  <div>
                    ...
                    <h3 className="text-base font-black text-slate-900 dark:text-slate-100 tracking-tight mt-0.5">
                      {trade.title}
                    </h3>
                  </div>
                ...
                <div className="mt-4 neu-inset p-3.5 rounded-2xl grid grid-cols-2 gap-3 text-xs">
                  <div>
                    <span className="text-[10px] font-extrabold uppercase tracking-wider text-orange-800 dark:text-orange-400 block">
                      Bolivian Exporter (BO)
                    </span>
                    <p className="font-black text-slate-900 dark:text-slate-100 truncate">{trade.sellerName}</p>
                    <p className="text-[11px] text-slate-600 dark:text-slate-400 font-medium">{trade.sellerCity}, Bolivia</p>
                  </div>

                  <div className="border-l border-slate-300/80 dark:border-slate-700/60 pl-3">
                    <span className="text-[10px] font-extrabold uppercase tracking-wider text-emerald-800 dark:text-emerald-400 block">
                      African Importer
                    </span>
                    <p className="font-black text-slate-900 dark:text-slate-100 truncate">{trade.buyerName}</p>
                ...
                {/* Amount & Yield Box */}
                <div className="mt-4 flex items-center justify-between neu-card-sm p-4 rounded-2xl border border-white/70 dark:border-white/10">
                  <div>
                    ...
                    <div className="text-[10px] text-slate-700 dark:text-slate-400 font-mono font-semibold">
              ...
              <div className="pt-2 border-t border-slate-300/50 dark:border-slate-800/80">
                {trade.status === 'CREATED' && (
                  <div className="flex gap-2">
                    <button
                      onClick={() => setSelectedTradeForFunding(trade)}
                      className="w-full py-3 px-4 rounded-2xl neu-btn-primary text-white text-xs font-bold flex items-center justify-center shadow-neu-glow"
                    >
                      Fund via M-Pesa / African Rail
                    </button>
                  </div>
                )}
                ...
                {trade.status === 'ESCROW_LOCKED' && (
                  <div className="flex flex-col sm:flex-row gap-2">
                    <button
                      ...
                      className="flex-1 py-3 px-4 rounded-2xl neu-btn-primary bg-gradient-to-r from-indigo-600 to-emerald-600 text-white text-xs font-bold flex items-center justify-center"
                    >
                      Confirm Inspection &amp; Release Escrow
                    </button>
                  </div>
                )}
                ...
                {trade.status === 'RELEASED' && (
                  <div className="flex gap-2">
                    <button
                      ...
                      className="w-full py-3 px-4 rounded-2xl neu-btn-primary bg-gradient-to-r from-orange-600 to-amber-600 text-white text-xs font-bold flex items-center justify-center shadow-neu-amber"
                    >
                      Cash Out {formatCurrency(bobAmount, 'BOB')} to Bank (Pollar Ramp)
                    </button>
                  </div>
                )}
      ...
      {filteredTrades.length === 0 && (
        <div className="neu-card rounded-3xl p-12 text-center space-y-3 border border-white/80 dark:border-white/10">
          <p className="text-slate-500 dark:text-slate-400 text-sm font-semibold">No trade contracts match your search.</p>
          <button
            onClick={() => {
              setFilterStatus('ALL');
              setSearchQuery('');
            }}
            className="px-4 py-2 rounded-2xl neu-btn text-xs font-bold text-slate-700 dark:text-slate-300"
          >
            Clear Filters
          </button>
        </div>
